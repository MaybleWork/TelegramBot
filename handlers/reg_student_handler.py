...
from samples.requests import student_reg_request
...
async def email_registration_callback_query(
    ...
    await state.set_state(UserRegistrationStates.reg_student.enter_email_login)
...
async def email_entering(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    await state.set_state(UserRegistrationStates.reg_student.enter_name)
...
async def name_entering(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    await state.set_state(UserRegistrationStates.reg_student.enter_last_name)
...
async def last_name_entering(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    await state.set_state(UserRegistrationStates.reg_student.enter_surname)
...
async def surname_entering(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    await state.set_state(UserRegistrationStates.reg_student.enter_password)
...
async def password_entering(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    result = await student_reg_request(
        collected_data={
    ...
    await edit_message_text_safe(
        bot=bot,
        text=templates.PASSWORD_STUDENT_REGISTRATION_MESSAGE
        if not result
    ...
    if not result:
        await state.set_state(UserRegistrationStates.reg_student.start_auth)
    else:
        await state.clear()
